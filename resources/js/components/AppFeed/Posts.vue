<template>
  <v-card class="mx-auto mb-4" outlined max-width="600">
    <v-card-text class="headline font-weight-medium pb-0 px-0">
      <p class="px-4 mb-0">
      <router-link :to="post.path" style="text-decoration: none" class="grey--text text--darken-2">
        {{ post.title | truncate(50) }}
      </router-link>
      </p>
      <div class="body-2 grey--text px-4 py-3 text--darken-2 postList">{{ post.body | striphtml | truncate(200)}}</div>
      <v-img
    ...
    <v-card-actions>
      <v-list-item class="grow">
        ...
        <v-row align="center" justify="end">
          <span class="caption mr-1 blue-grey--text">{{post.love_count}} Loves</span>

          <span class="mx-1"></span>

          <span class="caption mr-1 blue-grey--text">{{post.reply_count}} Comments</span>
        </v-row>
      </v-list-item>
    </v-card-actions>
  </v-card>
